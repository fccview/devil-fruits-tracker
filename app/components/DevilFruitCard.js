...
function getSpoilerSafeValue(fruit, field, number, type) {
  if (!fruit?.spoilers?.length) return fruit[field];

  const applicableSpoiler = fruit.spoilers
    .sort((a, b) => b.threshold[type] - a.threshold[type])
    .find(spoiler => number < spoiler.threshold[type]);
  ...
  if (applicableSpoiler) {
    ...
    return applicableSpoiler.previousValues[field] || fruit[field];
  }
